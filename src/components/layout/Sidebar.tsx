...
import BarLogo from "@/assets/icons/bar_logo";
...
const Sidebar = () => {
      ...
      <>
        {/* {isShowSideBar && ( */}
          <>
            ...
            ></div> */}
                  <aside className="w-[240px] bg-[#2E3A59] dark:bg-gray-800 shadow flex flex-col">

              <header className="filament-sidebar-header h-[3.5rem] shrink-0 border-b-1 border-gray-700 flex items-center justify-center relative bg-[#2E3A59]">
                <div className="flex items-center justify-center px-6 w-full">
                  <Link href="/" data-turbo="false" className="flex flex-row gap-2 w-full items-center">
                    <BarLogo />
                    <div className="filament-brand text-white  text-[14px] leading-none font-bold tracking-tighter">Bar Quest </div>
                  </Link>
                </div>
              </header>
  ...
  const SideBarItems = ({ link, icon: Icon, title, index }: ISideBarItems) => {
    ...
    const color = isActive || isHover ? '#ffff' : '#a0a9c0';
      ...
      <>
        <li className="filament-sidebar-item overflow-hidden">
          <div
            ...
            onClick={handleClick}
            className={`
              flex items-center justify-center gap-3 px-4 py-3 rounded-lg font-medium transition text-white cursor-pointer ${
          ...
          >
            ...
            <div className="flex flex-1">
              <span className="font-normal text-[14px] " style={{color: color}}>{title}</span>
            </div>
          </div>
        </li>
      </>
